...
public class DebugConsole : Singleton<DebugConsole>
{
    ...
    public static void AddCommand(CommandBase cmd, bool overrideIfCmdExists = false)
    {
        ...
        {
            Debug.LogError("Command ID is null or empty!", Instance.gameObject);
            return;
        }

        if (cmd.Id.Contains(" "))
        {
            Debug.LogError($"Command ID \"<b>{cmd.Id}</b>\" can not contain an empty space!", Instance.gameObject);
            return;
        }
            ...
            if (Instance._registeredCmds[i].Id == cmd.Id && Instance._registeredCmds[i].ParamsCount == cmd.ParamsCount)
            {
                if (overrideIfCmdExists)
                {
                    if (Instance._registeredCmds[i].IsConsoleNative)
                        Debug.LogError("Can not override a console native command!", Instance.gameObject);
                    else
                        Instance._registeredCmds[i] = cmd;
                }
                else
                    Debug.LogWarning($"A command with the same ID \"<b>{cmd.Id}</b>\" and the same parameters count (<b>{cmd.ParamsCount}</b>) is already registered!", Instance.gameObject);
    ...
    public static void RemoveCommand(string id, int paramsCount = -1)
    {
        ...
        for (int i = Instance._registeredCmds.Count - 1; i >= 0; --i)
        {
            ...
            if (paramsCount != -1)
            {
                ...
                if (Instance._registeredCmds[i].IsConsoleNative)
                {
                    Debug.LogWarning("Can not remove a console native command!", Instance.gameObject);
                    return;
                }
    ...
    private string ExtractCapitalLetters(string str, bool forceIncludeFirstChar = false)
    {
    ...
    private string RemoveLast(string str, int amount)
    {
    ...
    private string ConvertAutoCompletionToString()
    {
        ...
        if (!string.IsNullOrEmpty(_autoCompletionStr))
            _autoCompletionStr = RemoveLast(_autoCompletionStr, Constants.AUTO_COMPLETION_OPTIONS_SPLIT.Length);
    ...
    private string ToColored(string str, Color color)
    {
    ...
    private void OnGUI()
    {
        ...
        for (int i = _cmdsHistory.Count - 1; i >= 0; --i)
        {
            string cmdDisplay = ToColored(string.Format(Constants.ValidityFormats[_cmdsHistory[i].CmdValidity], _cmdsHistory[i].Cmd), _colorsByValidity[_cmdsHistory[i].CmdValidity]);
